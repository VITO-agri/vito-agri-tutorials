...
def nonzero_reducer(arr, mode):
    ...
    with warnings.catch_warnings():
        ...
        res = reducer_func(arr.data, axis=1)
...
def composite_ts(darr: xr.DataArray,
                 freq=7,
                 window=None,
                 start=None,
                 end=None,
    ...
    if mode in no_ov_modes:
        if window is not None and window != freq:
            print('`window` argument is ignored for '
    ...
    for i, d in enumerate(date_range):
        ...
        comp[:, i, ...] = nonzero_reducer(darr.isel(timestamp=idxs), mode)

    darr_out = xr.DataArray(comp,
                            dims=darr.dims,
                            coords={'variable': darr.coords['variable'].values,
                                    'timestamp': date_range.values,
                                    'y': darr.y,
                                    'x': darr.x},
                            attrs=darr.attrs)
